...
public class HolographicAPI {
            ...
            for (UUID uuid : packets.keySet()) {
                ...
                if (player != null && player.isOnline()) {
                    ...
                    if (loc.getWorld().getName().equals(location.getWorld().getName()) && loc.distanceSquared(location) < Math.pow(64, 2) && player != null && player.isOnline() && players.contains(uuid)) {
                        ...
                        if (BedwarsRel.getInstance().getCurrentVersion().startsWith("v1_8")) {
                            packet.getWatchableCollectionModifier().write(0, Collections.singletonList(new WrappedWatchableObject(2, title)));
                        } else {
    ...
    private Object getPacket(Location location) {
        try {
            ...
            Object instance = Utils.getNMSClass("EntityArmorStand").getConstructor(Utils.getNMSClass("World")).newInstance(cast.getClass().getMethod("getHandle", (Class<?>[]) new Class[0]).invoke(cast));
            if (title != null) {
